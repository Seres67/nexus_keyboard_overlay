...
typedef enum EAddonFlags {
  ...
  EAddonFlags_DisableHotloading = 2 /* prevents unloading at runtime, aka. will
                                       require a restart if updated, etc. */
} EAddonFlags;
...
typedef struct AddonDefinition {
  ...
  ADDON_UNLOAD Unload; /* Pointer to Unload Function of the addon. Not required
